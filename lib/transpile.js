...
const handleCommand = (statement) => {
  ...
  switch (commandArgs[0]) {
    ...
    case 'head':
    case 'body': {
      const props = commandArgs.slice(1)
      const componentChildren = commandBody || children.map(mapComponentChild)

      let el
      switch (commandArgs[0]) {
        case 'layout': el = 'div'; break
        case 'head': el = 'head'; break
        case 'body': el = 'body'; break
      }

      return new Layout(el, props, componentChildren)
    }
    default:
      return null
  }
